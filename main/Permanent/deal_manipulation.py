...
class Screenshot:
    ...
    def screenshot_helper(self, element_with_scroll, sub_section_name, deal_name, vertical=True):
        ...
        scroll_total = self.driver.execute_script(f"return arguments[0].scroll{scroll}",
                                                  element_with_scroll)
        content = self.driver.execute_script(f"return arguments[0].client{scroll}",
                                             element_with_scroll)
        ...
        while scroll_new < (scroll_total - 100):
            ...
            if vertical:
                self.driver.execute_script(f"arguments[0].scroll(0,{scroll_new});",
                                           element_with_scroll)
            else:
                self.driver.execute_script(f"arguments[0].scroll({scroll_new},0);",
                                           element_with_scroll)

            self.driver.get_screenshot_as_file(
                f'{main_folder_name}/{deal_name}/{sub_section_name} no. {count}.png')
    ...
    def screenshot(self, deal):
        ...
        edit_deal = 'https://' + deal.split('.', maxsplit=1)[0].split('/')[
            -1] + '.salestrekker.com/deal/edit/' + \
                ...
                try:
                    WdWait(self.driver, 30).until(
                        ec.visibility_of_element_located(
                            (By.CSS_SELECTOR, 'md-content > md-content')))
                except exceptions.TimeoutException:
                    print('Edit get timeout:', deal)
                    print(edit_deal)
                    traceback.print_stack()
                    self.driver.get_screenshot_as_file(
                        f'{self.driver.current_url} {datetime.now()}.png')
                    return
        ...
        deal_name = \
        self.driver.find_element(by=By.CSS_SELECTOR, value='header-title > h1').text.split(':',
                                                                                           maxsplit=1)[
            -1].lstrip()
        ...
        # deal_name = self.driver.find_element(by=By.CSS_SELECTOR,value='ticket-title > h1 > span').text
        ...
        try:
            ...
            credit_quote = self.driver.find_element(by=By.XPATH,
                                                    value="//button/span[contains(text(), 'quote')]")
            try:
                self.driver.find_element(by=By.XPATH,
                                         value="//button/span[contains(text(), 'quote')]").click()
            except exceptions.ElementClickInterceptedException:
                self.driver.execute_script('arguments[0].click();', credit_quote)
            WdWait(self.driver, 10).until(
                ec.visibility_of_element_located(
                    (By.CSS_SELECTOR, 'md-content > md-content > form-content > form')))
            content = WdWait(self.driver, 10).until(
                ec.visibility_of_element_located((By.CSS_SELECTOR, 'body > md-content')))
            try:
        ...
        test[-1].click()
        WdWait(self.driver, 20).until(
            ec.visibility_of_element_located((By.CSS_SELECTOR, 'st-contact')))
        try:
            for button_count, button in enumerate(
                    self.driver.find_elements(by=By.CSS_SELECTOR,
                ...
                if current_separator == 'Maximum borrowing':
                    try:
                        WdWait(self.driver, 15).until(
                            ec.visibility_of_element_located(
                                (By.TAG_NAME, 'st-maximum-borrowing main')))
                    except exceptions.TimeoutException:
                    ...
                    try:
                        WdWait(self.driver, 15).until(
                            ec.visibility_of_element_located(
                                (By.TAG_NAME, 'st-review-loan-products > st-block')))
                    except exceptions.TimeoutException:
                    ...
                    try:
                        content = WdWait(self.driver, 10).until(
                            ec.visibility_of_element_located(
                                (By.CSS_SELECTOR, '.sheet > div:nth-child(1)')))
                    except exceptions.TimeoutException:
                        pass
                    else:
                        self.screenshot_helper(element_with_scroll=content,
                    ...
                    for count, household_button in enumerate(
                            self.driver.find_elements(by=By.CSS_SELECTOR,
                                                      value='st-tabs-list-nav > button'), start=1):
                        if 'active' in household_button.get_attribute('class'):
                            pass
                        else:
                            household_button.click()
                        self.screenshot_helper(element_with_scroll=content,
                                               sub_section_name=f'{current_separator_text}{count}',
                                               deal_name=deal_name)
                    continue
